...
let mealObj = {
  ...
  avgDailyCalories: function(){
    let uniqueDays = [];
    uniqueDays.push(this.meals[0].date);
    for(let i = 1; i<this.meals.length; i++){
      if(this.meals[i].date !== this.meals[i-1].date){
      uniqueDays.push(this.meals[i].date);
      }
      }
    let numOfUniqueDays = uniqueDays.length;
    let totalCalories=0;
    for(let i = 0; i<uniqueDays.length; i++){
      totalCalories += this.caloriesEatenOn(uniqueDays[i]);
    }
    return totalCalories/numOfUniqueDays;
  },
};
